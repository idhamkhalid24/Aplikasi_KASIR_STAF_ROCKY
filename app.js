...
function truthyFlag(value){return value===true||value===1||String(value||'').toLowerCase()==='true'||String(value||'')==='1'}
function isTrialUser(user=state.user){const value=String(user?.accountType||user?.mode||user?.type||'').toLowerCase(),role=String(user?.role||'').toLowerCase(),username=key(user?.username||user?.id||user?.user||'');return !!user&&(truthyFlag(user.isDummy)||truthyFlag(user.trialMode)||truthyFlag(user.dummy)||truthyFlag(user.dummyMode)||truthyFlag(user.isTrial)||truthyFlag(user.trial)||truthyFlag(user.excludeFromReports)||truthyFlag(user.excludeFromReport)||value==='dummy'||value==='dumy'||value==='trial'||role==='dummy'||role==='dumy'||role==='trial'||username==='dummy'||username.startsWith('dummy_')||username.includes('_dummy')||username==='dumy'||username.startsWith('dumy_')||username.includes('_dumy')||username==='trial'||username.startsWith('trial_')||username.includes('_trial'))}
