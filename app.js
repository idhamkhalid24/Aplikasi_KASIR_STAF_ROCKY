...
async function loadStaffData(opts = {}) {
  ...
  try {
    // FASE 1 (wajib, bikin app kebuka cepat): cuma query yang sudah difilter
    // pakai index dateKey/monthKey bulan berjalan. Query "susulan" (legacy/tanpa
    // filter tanggal) dipindah ke loadStaffDataSupplemental() dan jalan di
    // belakang layar SETELAH layar utama tampil, supaya tidak ikut menahan buka app.
    // PENTING: tidak ada data yang dihilangkan, cuma urutan ambilnya diatur ulang.
    const [
      tx,
      att,
      prevAtt,
  ...
  // FASE 2 (background, TIDAK menahan tampilan): nyusul ambil data "legacy" /
  // tanpa filter tanggal, buat nangkep dokumen lama yang dateKey/monthKey-nya
  // belum lengkap. Ini yang dulu bikin data akhir bulan suka ketinggalan kalau
  // cuma andalin limit kecil. Sekarang tetap jalan tiap buka app, cuma tidak
  // lagi ikut bikin loading awal lama — hasilnya digabung diam-diam ke data
  // yang sudah tampil, dan layar di-refresh otomatis kalau ternyata ada
  // tambahan data.
  loadStaffDataSupplemental(u, m, prevD, monthStart, monthEnd).catch((e) =>
    console.warn("sync susulan gagal", e?.code || e),
  );
}
async function loadStaffDataSupplemental(u, m, prevD, monthStart, monthEnd) {
  if (!state.user || key(state.user.username) !== u) return;
  const [txLegacy, attLegacy, manualTarget, manualTrial, manualLegacy] =
    await Promise.all([
      getDocs(
        query(
          collection(db, "transactions"),
  ...
  // User bisa saja sudah ganti akun / logout selagi nunggu query susulan ini.
  if (!state.user || key(state.user.username) !== u) return;

  let changed = false;

  const mergedTx = mergeRowsById(
    state.data.tx,
    txLegacy.docs.map((x) => ({ id: x.id, ...x.data() })),
  ).filter((t) => txMonth(t) === m);
  if (mergedTx.length !== state.data.tx.length) {
    state.data.tx = sortDesc(mergedTx);
    seedTxProductHistoryFromTx(state.data.tx);
    changed = true;
  }

  const mergedAtt = dedupeAttendanceRows(
    mergeRowsById(
      state.data.att,
      attLegacy.docs.map((x) => ({ id: x.id, ...x.data() })),
    ).filter((a) => attDate(a).startsWith(m) || attDate(a) === prevD),
  );
  if (mergedAtt.length !== state.data.att.length) {
    state.data.att = mergedAtt;
    changed = true;
  }

  const legacyManualRows = mergeRowsById(
  ...
  ).filter((b) => manualBonusMonth(b) === m && !deleted(b));
  const mergedManual = mergeRowsById(state.data.manual, legacyManualRows);
  if (mergedManual.length !== state.data.manual.length) {
    state.data.manual = sortDesc(mergedManual);
    changed = true;
  }

  if (changed) {
    console.info(
      "[sync susulan] ada data lama tanpa dateKey lengkap yang baru ke-gabung.",
    );
    render();
  }
}
